...
struct Rect
{
	...
	Rect(const Point& position, const Point& size) : x(position.x), y(position.y), w(size.x), h(size.y) {}
	Rect(const Rect& r) : x(r.x), y(r.y), w(r.w), h(r.h) {}
	bool Contains(float px, float py) const {	return sf::FloatRect(x, y, w, h).contains(px, py); }
...
class QuadTree
{
	...
	Point* points;
public:
	QuadTree(const Rect& rect, int maxPoints = 4)
		: rect(rect), maxPoints(maxPoints), pointCount(0), points(new Point[maxPoints]) {}
	...
	bool Add(const Point& newPoint)
	{
		...
		{
			// Are sub-quads already initialized?
			if(!quads)
				InitQuads();
